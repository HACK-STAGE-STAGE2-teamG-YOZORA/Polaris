...
P0はデモPCを使う単一ユーザー・認証なしで動かす。P1ではGoogleログイン／新規登録／ログアウトとユーザーごとのデータ分離を実装する。System APIと認証開始・コールバック以外は有効なアプリ内セッションを必須とし、自己分析、経験、総合プロフィール、企業、ES、企業提案を認証ユーザー単位で保存・検索する。
...
flowchart TD
    ...
    B -- "ない" --> C["初回チャットを開始"]
    B -- "ある" --> D{"一覧から続きから／初めから"}
    D -- "続きからを選択" --> E["選んだセッションの会話履歴を復元"]
    D -- "初めから" --> F["他の進行中セッションを終わらせずに新規開始"]
    C --> G["AIと経験を一問ずつ深掘り"]
    E --> G
    F --> G
    G --> H["経験カード案"]
...
- 進行中セッションがある場合の大まかな案内(件数の一覧・個別の「続きから」は自己分析チャット画面側で行う)
- 「初めから」新しいセッションを始める導線
- 自己分析未実施時の案内文
...
- 保存済みESと最新の検査状態
- AIチャット、自己分析、ES添削、経験一覧、アカウントへ移動するタブ

セッション固有の結果は内部データとして上書きせず保存するが、ホームへ過去セッション一覧は表示しない。ホーム用総合傾向が古くなった場合は`STALE`と表示し、全履歴からの再集計を案内する。

ユーザーは複数の自己分析セッションを同時に進行できる。「進行中セッションが複数ある」状態そのものが通常の利用であり、異常状態ではない。
...
- 全完了セッションの自己分析結果と全確認済み経験を候補母集団とし、設問と文字数制限に最も合う根拠をAIが選ぶ。
- 候補履歴はDBへすべて保持する。AIのコンテキスト上限を超える場合は、ユーザー指定経験、設問との関連度、新しいレポートを優先して入力し、本文には関連する経験だけを使用する。
...
- ホームから初回開始、または自己分析チャット画面の一覧から複数の進行中セッションを個別に続きからできる。
- 「初めから」は他の進行中セッションを終わらせずに新規セッションを追加できる。過去の完了済み結果や確認済み経験を削除しない。
- 新セッションでは、他セッションの確認済み経験をそのセッション固有の4軸分析へ自動で混ぜない。他セッションの経験は経験一覧、ホーム総合傾向、ESの候補集合には残す。
...
- デモユーザーでのホーム
- LM Studio接続確認
- 初回／続きから／初めからのセッション管理(複数セッションの同時進行を含む)
- 自己分析チャット
- 経験カード生成・確認・修正
...
P1の面接機能は、確認済み経験と任意の本人確認済みES・公式企業情報から、根拠ID付きの深掘り質問と逆質問を同期生成する範囲とする。生成結果は永続化せず、音声入力や模擬面接の進行・評価はP2に含める。

Google認証はAuthorization Code Flow、PKCE、`state`検証を使用する。Googleの検証済み`sub`を外部識別子とし、アクセストークン・リフレッシュトークン・IDトークンは保存しない。アプリ内セッションは失効可能なHttpOnly Cookieとして実装する。

ユーザー所有リソースへ別ユーザーのIDを指定した場合は、存在有無を推測できないよう`403`ではなく`404 NOT_FOUND`を返す。企業、経験、自己分析レポートなどの関連IDを受け取る処理でも、すべて同一ユーザー所有であることを検証する。
