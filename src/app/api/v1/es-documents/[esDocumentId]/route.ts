...
export async function GET(request: Request, context: Context): Promise<Response> {
  try {
    ...
    const document = await prisma.esDocument.findFirst({ where: { id: esDocumentId, userId: auth.userId }, include: esDocumentInclude });
...
export async function PATCH(request: Request, context: Context): Promise<Response> {
  try {
    ...
    const exists = await prisma.esDocument.findFirst({ where: { id: esDocumentId, userId: auth.userId }, select: { id: true } });
    ...
    if (body.preferredExperienceIds !== undefined) {
      const checked = await validatePreferredExperiences(body.preferredExperienceIds, auth.userId);
