...
export async function createAnalysisSession(
  ...
  const session = await apiPost<AnalysisSession>("/analysis-sessions", body);
  // RESTART_ACTIVEでは既存の進行中セッションがABANDONEDになるため、一覧を作り直す
  invalidateAnalysisSessionCaches();
  return session;
}
...
// セッション一覧の取得件数。チャット画面と経験カード画面が同じ条件で取得し、
// CACHE_KEYS.analysisSessions のキャッシュを共有できるようにそろえている
...
export async function sendAnalysisMessage(
  ...
  const turn = await apiPost<ChatTurnResponse>(`/analysis-sessions/${sessionId}/messages`, body);
  // セッションの状態（READY_TO_FINALIZEへの遷移など）が変わるため一覧を作り直す
  invalidateAnalysisSessionCaches();
  return turn;
}
...
export async function generateAxisAssessments(sessionId: string): Promise<AxisAssessmentPage> {
  const page = await apiPost<AxisAssessmentPage>(
    `/analysis-sessions/${sessionId}/axis-assessments/generate`,
  );
  invalidateAnalysisSessionCaches();
  return page;
}
...
export async function finalizeAnalysisSession(sessionId: string): Promise<SelfAnalysisReport> {
  const report = await apiPost<SelfAnalysisReport>(`/analysis-sessions/${sessionId}/finalize`);
  invalidateAnalysisSessionCaches();
  return report;
}
...
export async function createExperienceDraft(
  ...
  const experience = await apiPost<ExperienceResponse>(
    `/analysis-sessions/${sessionId}/experience-drafts`,
    { experienceType, messageIds },
  );
  // 経験カードが増えるため、カード画面とES画面の根拠候補を作り直す
  invalidateExperienceCaches();
  return experience;
}

// POST /api/v1/overall-self-analysis/recompute — 総合自己分析プロファイルを再計算する
export async function recomputeOverallSelfAnalysis(): Promise<OverallSelfAnalysisProfileResponse> {
  const profile = await apiPost<OverallSelfAnalysisProfileResponse>("/overall-self-analysis/recompute");
  invalidateAnalysisSessionCaches();
  return profile;
}
